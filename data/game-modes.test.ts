...
describe("expanded game modes",()=>{
  ...
  it("provides at least 20 valid head-to-head questions",()=>{expect(headToHead.length).toBeGreaterThanOrEqual(20);for(const question of headToHead){expect([question.left.name,question.right.name]).toContain(question.answer);expect(question.left.value).not.toBe(question.right.value)}});
