...
function CareerRound({ again, seenIds, markSeen }: { again: () => void; seenIds: string[]; markSeen: (ids: string[]) => void }) {
  ...
  if (questions.length < 20 || !question) return <main className="game-shell"><section className="question-card"><div className="mode-label">CAREERS COMPLETE</div><h1>YOU’VE CLEARED EVERY FRESH CAREER PATH.</h1><p>We will not repeat paths you have already played. More verified careers are coming.</p></section></main>;
